# Train times from Hastings to London

Hastings connects to 1 London terminal. The fastest route is London Bridge in 85 minutes on a direct train.

- Fastest journey to London: 85 minutes to London Bridge
- Direct service: Yes
- London terminals served: 1 (London Bridge)
- Operator: Southeastern, Southern, Thameslink
- Source: Darwin Timetable Files (Rail Delivery Group), Open Government Licence v3.0
- Data reviewed: 2026-08-17

## Hastings to each London terminal

| London terminal | Fastest | Fastest direct | Typical peak | Peak trains/hr | Operator |
| --- | --- | --- | --- | --- |
| London Bridge | 85 min | same as fastest | 100 | 3.2 | Southeastern, Southern, Thameslink |

## Frequently asked questions

### How long does the train from Hastings to London take?

The fastest train from Hastings reaches London Bridge in 85 minutes. Hastings connects to 1 London terminal: London Bridge.

### Which London station should I travel to from Hastings?

London Bridge is the quickest at 85 minutes, on a direct service. Direct trains run to London Bridge.

### Is Hastings a good commuter town for London?

At 85 minutes to London Bridge, Hastings is a longer commute, typically traded off against more space and lower housing costs. Direct trains keep the journey simple.

### Are there direct trains from Hastings to London?

Yes. Direct services run to London Bridge.

### What are the nearest stations to Hastings?

The closest alternatives are St Leonards Warrior Square, West St Leonards, Crowhurst. These can offer a faster or cheaper route into London depending on where you live.

## About this data

Fastest is the quickest scheduled weekday service; typical peak is the median of
services arriving at the London terminal between 07:00 and 09:30. Computed from
Darwin Timetable Files published by the Rail Delivery Group under the Open
Government Licence. Not live times, and they exclude disruption and engineering
work. Full methodology: https://railreach.co.uk/about/

Source: RailReach - https://railreach.co.uk/stations/hastings/
Licence: CC BY 4.0. Please attribute RailReach and link to https://railreach.co.uk/
