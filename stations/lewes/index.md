# Train times from Lewes to London

Lewes connects to 3 London terminals. The fastest route is London Bridge in 70 minutes, with one change.

- Fastest journey to London: 70 minutes to London Bridge
- Direct service: No - one change required
- London terminals served: 3 (London Bridge, Victoria, St Pancras)
- Operator: Southeastern, Southern, Thameslink
- Source: Darwin Timetable Files (Rail Delivery Group), Open Government Licence v3.0
- Data reviewed: 2026-08-17

## Lewes to each London terminal

| London terminal | Fastest | Fastest direct | Typical peak | Peak trains/hr | Operator |
| --- | --- | --- | --- | --- |
| London Bridge | 70 min (change at Gatwick Airport) | no direct service | - | - | Southeastern, Southern, Thameslink |
| Victoria | 70 min | same as fastest | 71 | 1.2 | Southeastern, Southern |
| St Pancras | 85 min (change at Gatwick Airport) | no direct service | - | - | Thameslink, East Midlands Railway, Southeastern high speed |

## Frequently asked questions

### How long does the train from Lewes to London take?

The fastest train from Lewes reaches London Bridge in 70 minutes. Lewes connects to 3 London terminals: London Bridge, Victoria, St Pancras.

### Which London station should I travel to from Lewes?

London Bridge is the quickest at 70 minutes, though it requires a change. Direct trains run to Victoria.

### Is Lewes a good commuter town for London?

At 70 minutes to London Bridge, Lewes is a longer commute, typically traded off against more space and lower housing costs. Most services require one change.

### Are there direct trains from Lewes to London?

Yes. Direct services run to Victoria.

### What are the nearest stations to Lewes?

The closest alternatives are Cooksbridge, Glynde, Plumpton. These can offer a faster or cheaper route into London depending on where you live.

## About this data

Fastest is the quickest scheduled weekday service; typical peak is the median of
services arriving at the London terminal between 07:00 and 09:30. Computed from
Darwin Timetable Files published by the Rail Delivery Group under the Open
Government Licence. Not live times, and they exclude disruption and engineering
work. Full methodology: https://railreach.co.uk/about/

Source: RailReach - https://railreach.co.uk/stations/lewes/
Licence: CC BY 4.0. Please attribute RailReach and link to https://railreach.co.uk/
